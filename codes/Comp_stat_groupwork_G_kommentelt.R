###############################################################################
## Extreme Value Analysis of Tisza and Danube River Levels
## - Block Maxima Method (annual min & max)
## - GEV / Gumbel fitting with extRemes
## - Return levels for ~5000 days (≈13.7 years)
## - Likelihood ratio tests for changes between periods
###############################################################################

setwd("C:/Users/User/Desktop/Comp_stat_projekt") 

# Required packages -----------------------------------------------------------

library(readr)   # for reading csv files
library(dplyr)   # for data manipulation (group_by, summarise, mutate, etc.)
library(extRemes) # for extreme value modelling (fevd, return.level, lr.test, ci)


# 1. Read CSV files -----------------------------------------------------------

# Daily water levels for Tisza and Danube, split into two periods:
#  - 2002–2013 ("early")
#  - 2014–2024 ("late")
df_tisza_02_13 <- read_csv("river_data_tisza_02_13.csv")
df_tisza_14_24 <- read_csv("river_data_tisza_14_24.csv")
df_duna_02_13  <- read_csv("river_data_duna_02_13.csv")
df_duna_14_24  <- read_csv("river_data_duna_14_24.csv")
...
minmax_tisza_14_24 <- df_tisza_14_24 %>% 
  group_by(year) %>% 
  summarise(
...
minmax_tisza_02_13 <- df_tisza_02_13 %>% 
  group_by(year) %>% 
  summarise(
...
minmax_duna_02_13 <- df_duna_02_13 %>% 
  group_by(year) %>% 
  summarise(
    min_value = min(value, na.rm = TRUE),
    max_value = max(value, na.rm = TRUE)
...
minmax_duna_14_24 <- df_duna_14_24 %>% 
  group_by(year) %>% 
  summarise(
    min_value = min(value, na.rm = TRUE),
    max_value = max(value, na.rm = TRUE)
