...
# check how many missing values (NAs) we have
sum(is.na(river_data_tisza_02_13))
sum(is.na(river_data_tisza_14_24))
sum(is.na(river_data_duna_02_13))
sum(is.na(river_data_duna_14_24))  
# in the Duna 2014–2024 dataset there are 2 missing values
...
# we replace the NAs with i+-4 neighbours mean
for (i in na_idx) {
...
# based on the 03_GPD_initial_trying_not_final.R where we manually selected the threshold it was
# almost the same as th 95.perecentile, since selecting 4x2 threshold for each dataset would take much time
# we simply use the 95. percentile
